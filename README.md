...
基于 `@napi-rs/image` 的目录图片压缩工具。递归扫描所有子目录，保持文件扩展名和编码格式，只在输出严格小于原文件时执行原子替换。
...
| GIF | `@napi-rs/image` 不支持，明确跳过 |
| DDS | 只能解码、不能同格式编码，明确跳过 |
| OpenEXR | 只能解码、不能同格式编码，明确跳过 |
| SVG/SVGZ | 只能解码、不能同格式编码，明确跳过 |

WebP、AVIF 等输入支持会受到当前 `@napi-rs/image` 平台二进制能力影响。任何编码或回读验证失败都不会覆盖源文件。
